...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        model.train()
        stop_train = GLV.get_value_1('stop_train')
        if stop_train == 'True_stop':
            print("训练已经截至")
            break
...
def parse_opt(known=False):
    ...
    parser.add_argument('--patience', type=int, default=100, help='EarlyStopping patience (epochs without improvement)')#早停；如果模型在default值轮数里没有提升，则停止训练模型
    ...
    # Logger arguments
    parser.add_argument('--entity', default=None, help='Entity')#在线可视化工具，类似于tensorboard
...
def run(**kwargs):#传入的是关键字参数，关键字参数允许传入零个到任意个参数，它们在函数内部自动组装为一个字典 (dict)。
